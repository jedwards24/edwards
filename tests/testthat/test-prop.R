...
ref <- prop_ci(tb, "outcome", "group", plot = FALSE)
...
test_that("prop_ci handles inputs", {
  expect_identical(prop_ci(tb, "outf", "group", plot = FALSE), ref)
  expect_identical(prop_ci(tb, "outb", "group", plot = FALSE), ref)
  expect_identical(prop_ci(tb, "outbf", "group", pos_class = "TRUE", plot = FALSE), ref)
  expect_identical(prop_ci(tb, "outc", "group", pos_class = "yes", plot = FALSE), ref)
  expect_false(identical(prop_ci(tb, "outc", "group", plot = FALSE), ref))
  ...
  expect_error(prop_ci(tb2, "outcome", "group", plot = FALSE), "Target variable must be binary.")
})
